...
import { getSessionForDrill, type SessionRow } from "@/lib/db/sessions";
import { AGENT_PROFILES } from "@/lib/agent/profiles";

export default async function DrillReplayPage({
  params,
  searchParams,
}: {
  ...
  const session = await getSessionForDrill(user.id, id);
  if (!session) redirect(`/drill/${id}`);
